<template>
  <div>
    <Head title="User" />
    <h1 class="mb-8 text-3xl font-bold">User Manage</h1>
    <div class="flex items-center justify-between mb-6">
      <search-filter v-model="form.search" class="mr-4 w-full max-w-md" @reset="reset">
        ...
        <label class="block mt-4 text-gray-700">Trash:</label>
        <select v-model="form.trashed" class="form-select mt-1 w-full">
          <option :value="null" />
          <option value="with">with Trash</option>
          <option value="only">only Trash</option>
        </select>
      </search-filter>
